...
import './index.less';
import { QIndexSelect } from 'src/component';
import DATA from './data';
import { LocalStorage, getUrlParams } from 'src/common';
import LocIcon from 'src/assets/images/icon_loc.png';
...
function handleData(data: any) {
	...
	data.citys && data.citys.forEach(city => {
	...
	renderRow = (rowData: any, sectionID: string | number, rowID: string | number) => {
		return (
					<div className={`city ${handleItemStyle(sectionID)}`}  key={rowID} onClick={this.onSelectItem(rowData)}>
						{sectionID === '定' && <img className="icon-location-city" src={LocIcon} />}
						{rowData}
					</div>
				)
	};
